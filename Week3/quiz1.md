...
- 답: 1번은 브랜치의 변경 사항을 하나의 새로운 커밋으로 합치며, 2번은 모든 변경 사항을 하나의 커밋으로 압축하여 합치고, 3번은 브랜치의 변경 사항을 현재 브랜치 위에 재배치하고, 이후에 대상 브랜치를 합친다.
...
- 답: 커밋의 해시는 a1s2d3f이고,commit 메시지는 'docs: readme 추가'이다
...
- 답: revert, revert는 새로운 커밋을 생성하여 이전 커밋의 변경 사항을 되돌려 수정이 편하기 때문이다.
